...
This parser is not wired into the legacy JSON order-submission route. It is exposed through authenticated dry-run and preview endpoints, plus the explicitly paper-only `POST /webhooks/tradingview/pine/paper-submit` Phase 3 route. Dry-run parses and records the normalized command but never calls risk approval or a broker. Preview runs parser and server risk checks but also never submits.

The paper-submit route is receipt-time: it authenticates and caps the raw body at 4,096 bytes, obtains a fresh server-side market reference, applies the allowlist, quantity, notional, timestamp, and price-collar gates, and uses server-configured quantity rather than trusting Pine's requested size. It accepts only the trusted paper broker implementation and requires `PAPER_TRADING=true` plus `TRADING_ENABLED=true`. The parsed lifecycle/protection controls (`CANCEL_UNFILLED_AT_DEADLINE`, protective stops, and trails) are rejected until Phase 4 durable lifecycle persistence exists; no exits are submitted by this route.

Receipt-time submission has an accepted replay caveat: TradingView or a network intermediary can replay an alert after the broker accepted it but before the response was received. The route hashes the raw body for its durable idempotency key, persists the actual broker status and order ID, and retains `submission_ambiguous` records without release or automatic retry. Operators must inspect broker state before any manual action. This route has only been exercised with the in-process `FakeBroker` in tests; the repository's tests do not place external orders.
